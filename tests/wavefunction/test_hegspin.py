...
import fvmc
from fvmc.wavefunction.hegspin import PlanewaveSlaterSpin, PairJastrowCCKSpin
...
def test_planewave_slater_spin(cell, multi_det):
    model = PlanewaveSlaterSpin(cell=cell, multi_det=None)
...
def test_slater_spin_wick_eval(tmp_path):
    ...
    params['params']['PlanewaveOrbitalSpin_0']['VmapDense_0']['kernel'] = coeff
    ...
    cfg.sample.mala.steps = 100
    cfg.sample.mala.grad_clipping = 1.
    cfg.sample.mala.mass = (1.0, 100.) #AV add for spin
    ...
    n_iter = data.shape[0]
    ek_mean = data[:, 2].mean()
    ek_std = data[:, 2].std() / np.sqrt(n_iter)
    ep_mean = data[:, 1].mean()
    ep_std = data[:, 1].std() / np.sqrt(n_iter)
    et_mean = data[:, 3].mean()
    et_std = data[:, 3].std() / np.sqrt(n_iter)
    print("from fvmc:")
    ...
    n_q = qlist.shape[0]
    # w is coeff
    w = np.stack([coeffup, coeffdn], axis=0)
    n_orb = w.shape[2]
    ...
    def calc_e_kin(w):
        ...
        ekin = np.einsum('ijk, j ->', np.conjugate(w) * w, ekin_pref)
        return ekin
